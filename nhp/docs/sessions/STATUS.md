...
| WS-09 | Barcode scan-to-add meter (meter-type barcode label + scan wizard) | ✅ | 2026-06-16T03:10:27Z | 2026-06-16T03:29:02Z | (commit 4) |
...
- 2026-06-16T02:34:00Z spawned WS-08
...
- 2026-06-16T02:47:15Z gated WS-08 ✅ (`make smoke` runs the POC front to back on a throwaway --seed-dev backend: boot → collections → seed → seed-check → records-check (dashboard /records pipeline returns rows) → pnpm build green → pnpm test:unit 55/55; scripts/smoke.sh wires the existing checks + a new seed/records-check.mjs; nhp/README.md (run order, ports 8094/5194, .env creds, honest known-limitations) + nhp/docs/DEMO.md (login→gateway+30-net wizard→meter-type+bulk meters→auto-built dashboard) committed; no new TODOs — smoke surfaced no in-lane defect, only the sandbox-blocks-localhost env constraint (run unsandboxed); zero rubix diff, no stray rubix-data)
- 2026-06-16T02:47:15Z RUN COMPLETE — all 8 workstreams ✅ (WS-01..08). The NHP power-metering POC is built, demoable, and runs clean from a fresh checkout per nhp/README.md.
- 2026-06-16T03:29:02Z gated WS-09 ✅ (pnpm -C nhp/ui build green + 69/69 unit tests incl. barcode encode→decode→resolve round-trip + scan-wizard plan; meter-type carries a `nhp-mt:<key>` QR barcode rendered + printable in the admin (qrcode), a new scan-to-add-a-device wizard (camera via @zxing + manual fallback) resolves a scanned/typed code to its meter-type and writes a capacity-checked, tag-correct, register-stamped meter via /records; backfill-safe — seed types resolve from key with no migration; live-verified on --seed-dev:8094 — both seed types round-trip, stray codes rejected, scan write = 1 meter + 10/10 registers with the full tag chain in content.tags; zero rubix diff, no stray rubix-data)
