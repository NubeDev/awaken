...
| [BOARDS_REFLOW.md](features/BOARDS_REFLOW.md) | the flow runtime end to end: node palette (8), board JSON, execution models (one-shot `run()` vs the persistent scan `BoardEngine`), the async/push/quality-aware `PointAccess` seam, node-state `StatePolicy`, every board API incl. the SSE `/outputs/stream`, scheduler (interval + subscription), and the save-refire fix | updated (`bb775b1b`, 2026-06-14) — flow-runtime redesign |
...
| [DATASOURCE.md](features/DATASOURCE.md) | read-only native-SQL passthrough to external DBs (Timescale/PG): bound params, single-statement, caps (lenient/strict), named queries, registry, describe; **integrated** — `datasources.json` loader + HTTP routes + `datasource` board node (strict) + `{datasource, sql}` widget + read-only AI tools | library-verified + integrated (2026-06-13) — in-repo tests green; live-DB read through the stack still `#[ignore]` |
