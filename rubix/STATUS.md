...
- [~] **Outbound adapters**: the **MCP** adapter is shipped — `POST /api/v1/mcp`
      speaks JSON-RPC 2.0 (`initialize` / `tools/list` / `tools/call`) and
      dispatches tool calls into the same `build_tools_scoped` registry the
      embedded agent uses, so an external agent gets identical priority-array
      gating, tenant scope (a site-pinned principal confines the tools to its
      `{org}/{site}`), and HITL escalation. An escalation-band `write_point`
      suspends rather than committing: the held write persists to the `runs`
      table as origin `mcp` and resumes through the same
      `POST /api/v1/runs/{id}/resume` operator surface as a chat/dispatch run.
      Auth is the platform bearer middleware (PAT/service account on cloud).
      **Remaining:** A2A and AG-UI adapters are not present.
...
- [x] Capability enforcement **at the bus**: the server's queryables are
      scoped to owned sites (a node only answers for keys it holds), and a
      driver confines its **own** session to its granted prefixes — the
      `ScopedSession` wrapper (`rubix-driver-sim`) authorizes every publish and
      subscribe against the driver's `CapabilitySet` before it reaches the bus,
      so an out-of-scope keyexpr is refused locally (named `DriverError::Denied`)
      rather than silently dropped by the bus responder. The reference driver
      publishes `cur` through it; a live supervisor-spawn test proves an
...
- [x] A reference driver binary (`rubix-driver-sim`): capability-scoped zenoh
      ...
      the supervisor in a live test — including a live out-of-grant-publish
      refusal test (the sim attaches but no `cur` reaches a peer).
...
- Supervisor spawn is now covered by a live end-to-end test (the real
  `Supervisor` spawns the `rubix-driver-sim` binary, which attaches and
  publishes, and a sibling test proves an out-of-grant publish is refused
  locally); restart-after-crash is still not exercised live. Board execution is
  tested (single-shot run over the store), but not a scheduled/triggered board.
