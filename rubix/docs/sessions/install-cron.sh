#!/usr/bin/env bash
# Installs (or removes) the cron entry that drives the rubix backend build loop every 5 minutes.
...
#
# Kill switch (no crontab edit needed):  touch rubix/docs/sessions/.loop.STOP
# Resume:                                rm   rubix/docs/sessions/.loop.STOP
set -euo pipefail

TICK="/home/user/code/rust/awaken/rubix/docs/sessions/loop-tick.sh"
LOG="/home/user/code/rust/awaken/rubix/docs/sessions/cron.log"
MARKER="# rubix-build-loop"
LINE="*/5 * * * * $TICK $MARKER"
...
case "${1:-install}" in
  ...
  remove)
    ...
    echo "Removed the rubix-build-loop cron entry."
    ;;
  status)
