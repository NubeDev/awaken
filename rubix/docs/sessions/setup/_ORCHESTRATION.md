...
The rubix BMS/EMS backend (`rubix/`, a standalone Cargo workspace, not part of the awaken
workspace). The build target is [STACK-DEISGN.md](../../../STACK-DEISGN.md); the gap inventory is
[STATUS.md](../../../STATUS.md) ("Not started / remaining"). The queue in
[STATUS.md](./STATUS.md) (this directory) decomposes those gaps into workstreams.
...
**commits before the next starts**, so a later session finds its dependencies (e.g. WS-02's run
store) already sitting in the working tree — dependencies resolve for free, no merging. The cost is
...
4. **DONE GATE** (before marking any WS ✅ — this is how we trust a session finished):
   ...
   - If DTOs/handlers changed, the OpenAPI surface still builds (`/api-docs/openapi.json` route
     compiles; utoipa derives are in sync).
...
```
You are implementing <WS-xx> for the rubix BMS/EMS backend, as one autonomous session in an
unattended build. You run to completion and return — you cannot ask the human anything.

READ FIRST, IN ORDER:
1. rubix/../CLAUDE.md                               (the coding standard — governs every file)
2. rubix/../STACK-DEISGN.md                         (the platform target — why this matters)
3. rubix/../STATUS.md                               (the gap inventory — your gap lives here)
4. rubix/docs/sessions/<WS-xx>.md                   (your spec — source of truth for scope)
5. rubix/docs/sessions/STATUS.md                    (what's already done — your deps are committed)
...
- STAY IN YOUR LANE: edit the files your WS owns. Touch a shared file (main.rs, lib.rs, api/mod.rs,
  store/schema.rs, the error enum) only as a minimal additive change. If a contract you depend on is
  missing, that's a TODOs.md blocker — do not redefine it.
...
- If you add/change env vars, DTOs, or schema, update STATUS.md's env list and the OpenAPI registration.
...
claim drifted, fix the WS doc + bump its `Verified:` line BEFORE coding. Then confirm your
dependency WSs are committed in the tree. Then implement incrementally, commit (messages prefixed
`<WS-xx>:` and following the commit convention), and update STATUS.md + your session doc. When done,
ensure the build/tests/clippy are green and return a concise summary of what landed and what (if
anything) you logged to TODOs.md.
```
